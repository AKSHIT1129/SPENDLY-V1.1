...
class FinanceViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private suspend fun seedDatabase(primaryName: String) {
        ...
        repository.insertTransaction(Transaction(amount = 120000.0, category = "Salary", description = "$primaryName Monthly Base", date = now - 5 * 24 * 60 * 60 * 1000, memberId = 1, isShared = false))
